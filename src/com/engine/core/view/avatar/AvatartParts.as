package com.engine.core.view.avatar
{
	...
	public class AvatartParts extends Proto 
	{
		...
		public function bodyRender(now:Boolean=false):void
		{
			...
			if (this.isFlyMode && this.isOnMonut && playAction != ActionConst.MEDITATION && playAction != ActionConst.SKILL2) {
				playAction = ActionConst.STAND;
			}
			if (this.specialMode == 1) {
				playAction = ActionConst.SKILL2;
			}
			...
			if (actionDict) {
				...
				if (this.runing == false && this.state == ActionConst.WALK) {
					this.state = ActionConst.STAND;
				}
				var interval:int = _speed - this.vs;
				if (this.state == ActionConst.ATTACK || this.state == ActionConst.QUNGONG || this.state == ActionConst.SKILL1 || this.state == ActionConst.MEDITATION) {
				...
				if (passTime >= interval || now) {
					...
					for each (param in actionDict) {
						...
						if (_currentFrame >= param.frames && (!this.isFlyMode || playType != AvatarTypes.MOUNT_TYPE)) {
							...
							} else if (_currRestrict && _currRestrict.state && playAction != ActionConst.DEATH && playAction != ActionConst.MEDITATION) {
								if (this.playEndFunc != null) {
									this.playEndFunc(callParam);
								}
		...
		public function effectRender():void
		{
			...
			if (this.lockEffectState) {
				actionDict = this.effectsParts[ActionConst.STAND] as Dictionary;
			} else {
		...
		public function removeEffect(itemType:String):String
		{
			...
			if (this.effectsParts) {
				var dict:Dictionary = this.effectsParts[ActionConst.STAND];
